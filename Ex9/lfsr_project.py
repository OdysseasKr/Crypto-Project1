"""
 ...
 TESTS:In the example below we use the function 'lfsr' in order to compute the
 keysrteam of an lfsr given the initial seed : [1,1,0,0]
 and the feedback function : [0,0,1,1], which correspomds to the feedback
 polynomial x^4+x^3+1. The third argument is an integer, which
...
Also the function string_xor( a binary string,a binary string) add bit by bit
the bits from the two strings
Example:
>key='1'*25;
